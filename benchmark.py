...
def benchmark_sequence(n_values):
    print(f"Benchmarking Fibonacci sequence generation:")
    print("-" * 60)
    ...
    for n in n_values:
        ...
        print(f"fibonacci_sequence({n:4d}) | Length: {len(result):4d} | Time: {execution_time:8.3f} ms")
...
def compare_implementations():
    ...
    try:
        ...
        for i, n in enumerate(test_values):
            speedup = python_times[i] / cpp_times[i] if cpp_times[i] > 0 else float('inf')
            print(f"{n:6d} | {python_times[i]:12.3f} | {cpp_times[i]:12.3f} | {speedup:9.2f}x")
